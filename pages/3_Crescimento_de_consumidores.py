...
st.title('Analise de crescimentos de consumidores')
...
def load_data(selected_states, consumption_type):
    ...
    for state in selected_states:
        ...
        filtered_data = filtered_data[ ~((filtered_data['ano'] == 2021))]  # Filter out 2004 and 2021
        yearly_data = filtered_data.groupby('ano')['numero_consumidores'].sum().reset_index()
        yearly_data['crescimento'] = yearly_data['numero_consumidores'].diff(1).fillna(0)
        yearly_data['sigla_uf'] = state
        growth_data.extend(yearly_data.to_dict('records'))
        growth_mean.append({
            'sigla_uf': state,
            'crescimento_avg': f"{yearly_data.get('crescimento').mean():,.2f}"
        })
    ...
    st.markdown("Media de crescimentos")
...
def load_screen():
    ...
    with col2:
        consumption_type = st.selectbox('Consumo', options=list(df['tipo_consumo'].unique()), index=2)
